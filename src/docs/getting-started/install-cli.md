---
title: Install deployctl
description: Install the Shipwick command-line client on a laptop or in CI, and connect it to an agent.
---

# Install deployctl

`deployctl` is the Shipwick command-line client. This page covers installing it on your laptop or in CI, saving the agent's URL and token with `deployctl login`, and how the CLI decides which agent to talk to.

The server installer already puts `deployctl` on the server. You only need this page for other machines.
...
Download `deployctl_windows_amd64.exe` from the [releases page](https://github.com/shipwick/shipwick/releases) and put it on your `PATH`.
...
`deployctl login` saves the agent's URL and the API token for later commands.
...
The saved token belongs to the saved URL. If `--url` or `SHIPWICK_AGENT_URL` points `deployctl` at a different agent, the saved token is not sent there; set `SHIPWICK_AGENT_TOKEN` for that agent, or log in to it.

## How deployctl handles the token

The API token is equivalent to root SSH access to the server, and `deployctl` treats it that way:
...
- `deployctl` warns on standard error whenever the token is about to travel over plain HTTP to anything other than the local machine. Use HTTPS or an SSH tunnel.

## What's next

- [Deploy your first application](/docs/getting-started/first-deployment).
- The full command list is in the [deployctl reference](/docs/reference/deployctl).
