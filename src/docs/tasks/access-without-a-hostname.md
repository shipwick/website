...
The agent's API does not have to be reachable from the internet. This page shows how to leave it unexposed and use `deployctl` through an SSH tunnel or a private network instead. It is for servers where you have no hostname to spare for the API, or prefer not to publish it.
...
```text
  No API hostname was set, so the API is not exposed. Reach it through a tunnel:
    ...
    2. on your laptop:   ssh -N -L 9000:127.0.0.1:9000 root@<this-server> &   deployctl login
```
...
Then log in. The default URL of `deployctl` is `http://127.0.0.1:9000`, which is the tunnel:
...
Press Enter to accept the URL, and paste the token. From then on every `deployctl` command works as long as the tunnel is open. Without the tunnel, `deployctl` reports that it cannot reach the agent and reminds you of the `ssh -L` command.

`deployctl` does not warn about plain HTTP here: the token only travels to your own machine's loopback address, and SSH encrypts it from there.
...
Then point `deployctl` at it:

```bash
deployctl login --url http://<private address>:9000
```

`deployctl` warns whenever a token is about to travel over plain HTTP to anything other than the local machine, and it will do so here. It cannot know that the network underneath is encrypted. Make sure that it is, and that the address is not reachable from outside the private network.
